...
function buildGeneratedSqlQuestionBank(total = 420) {

  const topics = [
    "SELECT filtering", "JOIN conditions", "GROUP BY rules", "HAVING usage", "subquery correlation",
    "window functions", "CTE behavior", "index usage", "normalization", "transaction isolation",
    "NULL semantics", "aggregate behavior", "ordering guarantees", "set operators", "constraints",
    "foreign keys", "execution plans", "sargability", "locking behavior", "deadlock handling",
    "upsert patterns", "pagination strategy", "data consistency", "schema evolution", "query optimization",
    "date/time functions", "string functions", "materialized results", "view behavior", "security permissions"
  ];

  const lenses = [
    "fundamental", "intermediate", "advanced", "debug", "design",
    "performance", "production", "review", "edge-case", "consistency",
    "optimization", "maintenance", "analytics", "architecture"
  ];
  ...
  for (let i = 0; i < total; i += 1) {
    ...
    result.push({
      ...
      question: `SQL ${lens} check ${i + 1}: Which statement about ${topic} is correct?`,

      answers: [

        `For ${topic}, apply exact SQL semantics and verify result-set behavior carefully.`,

        `For ${topic}, database optimizer guarantees identical plans and costs always.`,

        `For ${topic}, NULL and join edge cases can be ignored if query runs.`,

        `For ${topic}, constraints and transaction isolation do not affect correctness.`

      ],

      correct: 0,

      explanation: `Correct: ${topic} requires precise SQL rule-based reasoning and validation.`
...
function pickDisjointPool(levelKey, sourcePools, usedKeys, total = 25, fillerOffset = 0) {
  ...
  if (selected.length < total) {
    ...
    const filler = generatedSqlQuestionBank.slice(fillerOffset).concat(generatedSqlQuestionBank.slice(0, fillerOffset));
...
const generatedSqlQuestionBank = buildGeneratedSqlQuestionBank(420);

const usedSqlQuestionKeys = new Set();

const disjointLevelPools = {

  l1: pickDisjointPool("l1", [l1QuestionPool], usedSqlQuestionKeys, 25, 0),

  l2: pickDisjointPool("l2", [l2QuestionPool], usedSqlQuestionKeys, 25, 40),

  l3: pickDisjointPool("l3", [l3QuestionPool], usedSqlQuestionKeys, 25, 80),

  l4: pickDisjointPool("l4", [l4QuestionPool], usedSqlQuestionKeys, 25, 120),

  l5: pickDisjointPool("l5", [l1QuestionPool, l2QuestionPool], usedSqlQuestionKeys, 25, 160),

  l6: pickDisjointPool("l6", [l2QuestionPool, l3QuestionPool], usedSqlQuestionKeys, 25, 200),

  l7: pickDisjointPool("l7", [l3QuestionPool, l4QuestionPool], usedSqlQuestionKeys, 25, 240),

  l8: pickDisjointPool("l8", [l4QuestionPool], usedSqlQuestionKeys, 25, 280),

  l9: pickDisjointPool("l9", [l3QuestionPool, l4QuestionPool], usedSqlQuestionKeys, 25, 320),

  l10: pickDisjointPool("l10", [l4QuestionPool, l3QuestionPool], usedSqlQuestionKeys, 25, 360)
