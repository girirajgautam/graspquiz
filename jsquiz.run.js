...
function buildGeneratedJsQuestionBank(total = 420) {

  const topics = [
    "scope rules", "closures", "prototype chain", "event loop", "promise chaining",
    "async/await", "DOM updates", "array methods", "object mutation", "immutability patterns",
    "error handling", "module boundaries", "strict mode", "hoisting behavior", "equality rules",
    "type coercion", "performance profiling", "memory leaks", "fetch lifecycle", "abort handling",
    "this binding", "class semantics", "iteration protocols", "concurrency limits", "security basics",
    "state management", "functional patterns", "testing strategy", "build tooling", "runtime debugging"
  ];

  const lenses = [
    "fundamental", "intermediate", "advanced", "debug", "design",
    "performance", "production", "edge-case", "review", "reliability",
    "maintenance", "runtime", "architecture", "testing"
  ];
  ...
  for (let i = 0; i < total; i += 1) {
    ...
    result.push({
      ...
      question: `JavaScript ${lens} check ${i + 1}: Which statement about ${topic} is correct?`,

      answers: [

        `For ${topic}, follow exact JavaScript semantics and validate assumptions explicitly.`,

        `For ${topic}, runtime behavior is always identical across environments without checks.`,

        `For ${topic}, edge cases can be ignored if lint passes.`,

        `For ${topic}, coercion and mutability never affect correctness.`

      ],

      correct: 0,

      explanation: `Correct: ${topic} requires precise JavaScript rule-based reasoning.`
...
function pickDisjointPool(levelKey, sourcePools, usedKeys, total = 25, fillerOffset = 0) {
  ...
  if (selected.length < total) {
    ...
    const filler = generatedJsQuestionBank.slice(fillerOffset).concat(generatedJsQuestionBank.slice(0, fillerOffset));
...
const generatedJsQuestionBank = buildGeneratedJsQuestionBank(420);

const usedJsQuestionKeys = new Set();

const disjointLevelPools = {

  l1: pickDisjointPool("l1", [l1QuestionPool], usedJsQuestionKeys, 25, 0),

  l2: pickDisjointPool("l2", [l2QuestionPool], usedJsQuestionKeys, 25, 40),

  l3: pickDisjointPool("l3", [l3QuestionPool], usedJsQuestionKeys, 25, 80),

  l4: pickDisjointPool("l4", [l4QuestionPool], usedJsQuestionKeys, 25, 120),

  l5: pickDisjointPool("l5", [l1QuestionPool, l2QuestionPool], usedJsQuestionKeys, 25, 160),

  l6: pickDisjointPool("l6", [l2QuestionPool, l3QuestionPool], usedJsQuestionKeys, 25, 200),

  l7: pickDisjointPool("l7", [l3QuestionPool, l4QuestionPool], usedJsQuestionKeys, 25, 240),

  l8: pickDisjointPool("l8", [l4QuestionPool], usedJsQuestionKeys, 25, 280),

  l9: pickDisjointPool("l9", [l3QuestionPool, l4QuestionPool], usedJsQuestionKeys, 25, 320),

  l10: pickDisjointPool("l10", [l4QuestionPool, l3QuestionPool], usedJsQuestionKeys, 25, 360)
