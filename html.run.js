...
function buildGeneratedHtmlQuestionBank(total = 420) {

  const topics = [
    "document structure", "semantic tags", "form attributes", "accessibility labels", "media elements",
    "table structure", "metadata tags", "link behavior", "script loading", "layout containers",
    "ARIA usage", "heading hierarchy", "interactive controls", "input validation", "iframe safety",
    "resource hints", "SEO semantics", "progressive enhancement", "fallback content", "doctype impact",
    "content models", "global attributes", "custom data attributes", "navigation landmarks", "figure semantics",
    "microdata basics", "loading behavior", "error handling patterns", "cross-browser behavior", "maintainability"
  ];

  const lenses = [
    "fundamental", "intermediate", "advanced", "debug", "accessibility",
    "performance", "production", "review", "edge-case", "standards",
    "seo", "architecture", "testing", "maintenance"
  ];
  ...
  for (let i = 0; i < total; i += 1) {
    ...
    result.push({
      ...
      question: `HTML ${lens} check ${i + 1}: Which statement about ${topic} is correct?`,

      answers: [

        `For ${topic}, follow semantic HTML rules and standards-compliant behavior.`,

        `For ${topic}, semantics and accessibility can be ignored if page renders.`,

        `For ${topic}, browser defaults always guarantee best behavior.`,

        `For ${topic}, attribute correctness does not affect interoperability.`

      ],

      correct: 0,

      explanation: `Correct: ${topic} requires standards-based, semantic HTML decisions.`
...
function pickDisjointPool(levelKey, sourcePools, usedKeys, total = 25, fillerOffset = 0) {
  ...
  if (selected.length < total) {
    ...
    const filler = generatedHtmlQuestionBank.slice(fillerOffset).concat(generatedHtmlQuestionBank.slice(0, fillerOffset));
...
const generatedHtmlQuestionBank = buildGeneratedHtmlQuestionBank(420);

const usedHtmlQuestionKeys = new Set();

const disjointLevelPools = {

  l1: pickDisjointPool("l1", [l1QuestionPool], usedHtmlQuestionKeys, 25, 0),

  l2: pickDisjointPool("l2", [l2QuestionPool], usedHtmlQuestionKeys, 25, 40),

  l3: pickDisjointPool("l3", [l3QuestionPool], usedHtmlQuestionKeys, 25, 80),

  l4: pickDisjointPool("l4", [l4QuestionPool], usedHtmlQuestionKeys, 25, 120),

  l5: pickDisjointPool("l5", [l1QuestionPool, l2QuestionPool], usedHtmlQuestionKeys, 25, 160),

  l6: pickDisjointPool("l6", [l2QuestionPool, l3QuestionPool], usedHtmlQuestionKeys, 25, 200),

  l7: pickDisjointPool("l7", [l3QuestionPool, l4QuestionPool], usedHtmlQuestionKeys, 25, 240),

  l8: pickDisjointPool("l8", [l4QuestionPool], usedHtmlQuestionKeys, 25, 280),

  l9: pickDisjointPool("l9", [l3QuestionPool, l4QuestionPool], usedHtmlQuestionKeys, 25, 320),

  l10: pickDisjointPool("l10", [l4QuestionPool, l3QuestionPool], usedHtmlQuestionKeys, 25, 360)
